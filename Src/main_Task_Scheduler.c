...
void tasks_Init(void) {
	...
	user_Tasks[0].top_Of_Stack = TASK_IDLE_STACK_START;
	user_Tasks[1].top_Of_Stack = TASK_1_STACK_START;
	user_Tasks[2].top_Of_Stack = TASK_2_STACK_START;
	user_Tasks[3].top_Of_Stack = TASK_3_STACK_START;
	user_Tasks[4].top_Of_Stack = TASK_4_STACK_START;
	uint32_t *pPSP;
	for(int i = 0; i < MAX_TASKS; i++) {
		pPSP = (uint32_t*)user_Tasks[i].top_Of_Stack;
		pPSP--;
		...
		*pPSP = EXEC_RETURN;	// LR = Exec return value
		...
		for(uint8_t j = 0; j<13; j++) {	// Initializing registers R4-R11 to 0
			pPSP--;
			*pPSP = 0;
		}
...
void unblock_tasks(void) {
	for(int i = 1; i<MAX_TASKS; i++) {
		if(user_Tasks[i].task_State != ready) {
...
void get_next_task_to_run(void) {
	Task_States_t state = blocked;

	for(int i = 0; i< MAX_TASKS; i++) {
		current_Task++;
		current_Task = current_Task % MAX_TASKS;
		state = user_Tasks[current_Task].task_State;
